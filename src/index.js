...
app.get('/manual', (req, res) => {
  ...
  res.send(
    ...
    При возникновении проблем с проверкой какого-либо функционала, сообщите пожалуйста в чат. Возможно внедрён новый куки, который не перехватывается или не обрабатывается на сервере. <br>
    <p>1) Для работы пропишите в файле config.ts:</p>
    <p>var workHost = <b>"https://simple-proxy-server-03.onrender.com"</b></p>
    <p style="max-width: 50%">1-б) Для локальной работы (только для симулятора IOS или десктопных браузеров в обычном режиме; панель управления - http://localhost:3000/manual) запустите прокси-сервер (npm start), пропишите в файле config.ts: </p>
